...
fn test_order_placement_cancelation_and_market_prices() {
	...
	let expected_account_balance = U128(29999999999999999999990000);
	...
	accounts[0].cancel_order(&mut runtime, U64(0), U64(1), U128(1)).expect("order cancelation failed");
	accounts[0].cancel_order(&mut runtime, U64(0), U64(1), U128(0)).expect("order cancelation failed");

	let expected_account_balance = U128(30000000000000000000000000);
