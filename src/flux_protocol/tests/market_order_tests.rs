...
fn test_place_order_insufficient_funds() {
	...
	let account_1_res = accounts[1].place_order(&mut runtime, U64(0), 0, U128(50000), U128(50), None);
...
fn test_order_placement_cancelation_and_market_prices() {
	...
	let tx_res = accounts[0].place_order(&mut runtime, U64(0), 1, U128(1000), U128(50), None).expect("tx unexpectedly failed");
	let tx_res = accounts[0].place_order(&mut runtime, U64(0), 1, U128(1000), U128(50), None).expect("tx unexpectedly failed");
	
	let no_market_price = accounts[0].get_market_price(&mut runtime, U64(0), 0);
	assert_eq!(no_market_price, U128(50));
	
	accounts[0].place_order(&mut runtime, U64(0), 1, U128(1000), U128(60), None).expect("tx unexpectedly failed");

	let no_market_price = accounts[0].get_market_price(&mut runtime, U64(0), 0);
	assert_eq!(no_market_price, U128(40));

	accounts[0].cancel_order(&mut runtime, U64(0), 1, U128(60), U128(2)).expect("order cancelation failed");
	...
	let tx_res = accounts[0].cancel_order(&mut runtime, U64(0), 1, U128(50), U128(1)).expect("order cancelation failed");
	let tx_res = accounts[0].cancel_order(&mut runtime, U64(0), 1, U128(50), U128(0)).expect("order cancelation failed");
