...
fn test_custom_gas_txs() {
	...
	alice.place_order(&mut runtime, U64(0), 0, U128(100000000000), 50, None, Some(vec![U64(SINGLE_CALL_GAS), U64(SINGLE_CALL_GAS)])).unwrap();    
    alice.cancel_order(&mut runtime, U64(0), 0, 50, U128(0), Some(vec![U64(SINGLE_CALL_GAS)])).unwrap();    
	alice.place_order(&mut runtime, U64(0), 0, U128(100000000000), 50, None, Some(vec![U64(SINGLE_CALL_GAS), U64(SINGLE_CALL_GAS)])).unwrap();    
	alice.place_order(&mut runtime, U64(0), 0, U128(100000000000), 50, None, Some(vec![U64(SINGLE_CALL_GAS), U64(SINGLE_CALL_GAS)])).unwrap();    
	alice.place_order(&mut runtime, U64(0), 1, U128(100000000000), 50, None, Some(vec![U64(SINGLE_CALL_GAS), U64(SINGLE_CALL_GAS)])).unwrap();    
