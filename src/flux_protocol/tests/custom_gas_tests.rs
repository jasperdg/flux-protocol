...
fn test_custom_gas_txs() {
	...
	alice.inc_allowance(&mut runtime, flux_protocol(), U128(to_dai(500))).expect("allowance couldn't be set");
	root.inc_allowance(&mut runtime, flux_protocol(), U128(to_dai(500))).expect("allowance couldn't be set");
