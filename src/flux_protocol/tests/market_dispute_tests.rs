...
fn test_dispute_valid() {
	...
	alice.place_order(&mut runtime, U64(0), 0, U128(to_dai(1) / 10), U128(70), None).expect("order placement failed unexpectedly");
	alice.place_order(&mut runtime, U64(0), 3, U128(to_dai(1) / 10), U128(10), None).expect("order placement failed unexpectedly");
	
	carol.place_order(&mut runtime, U64(0), 1, U128(to_dai(1) / 10), U128(10), None).expect("order placement failed unexpectedly");
	carol.place_order(&mut runtime, U64(0), 2, U128(to_dai(1) / 10), U128(10), None).expect("order placement failed unexpectedly");

	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(1))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	carol.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(4))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	
	let tx_res = alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(10))).expect("market dispute failed unexpectedly"); 
...
fn test_market_not_resoluted() {
	...
	alice.transfer(&mut runtime, carol.get_account_id(), to_dai(30).into()).expect("transfer failed couldn't be set");
	alice.set_allowance(&mut runtime, flux_protocol(), U128(to_dai(30))).expect("allowance couldn't be set");
	alice.dispute_market(&mut runtime, U64(0), Some(0), U128(to_dai(5))).expect("dispute failed");
}
...
fn test_finalized_market() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(5))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
...
fn test_dispute_after_dispute_window() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns() + 43200000000000;
	let dispute_res = alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(5))).expect("dispute failed");
...
fn test_finalize_as_not_owner() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(5))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(10))).expect("dispute failed");

	runtime.current_block().block_timestamp = market_end_timestamp_ns() + 43200000000000;
	let tx_res = alice.finalize_market(&mut runtime, U64(0), Some(0)).expect("market finalization failed as expected"); 
}
...
fn test_invalid_dispute_outcome() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	alice.resolute_market(&mut runtime, U64(0), Some(4), U128(to_dai(5))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
}
...
fn test_dispute_with_same_outcome() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(5))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	alice.dispute_market(&mut runtime, U64(0), Some(0), U128(to_dai(10))).expect("dispute failed");
}
...
fn test_dispute_escalation_failure() {
	...
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(5))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(10))).expect("dispute failed");
	alice.dispute_market(&mut runtime, U64(0), Some(2), U128(to_dai(20))).expect("dispute failed");
}
...
fn test_stake_refund() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(7))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	...
	alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(15))).expect("dispute failed");
...
fn test_insufficient_balance() {
	...
	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	let tx_res = alice.resolute_market(&mut runtime, U64(0), Some(0), U128(ntoy(101))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
} 
...
fn test_cancel_dispute_participation() {
	...
	alice.resolute_market(&mut runtime, U64(0), Some(1), U128(to_dai(4))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(5))).expect("market resolution failed unexpectedly"); // carol resolutes correctly - should have 1 % of 10 dai as claimable 
	
	alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(10))).expect("market dispute failed unexpectedly"); 
	...
	alice.withdraw_dispute_stake(&mut runtime, U64(0), U64(0), Some(1)).expect("dispute stake claim failed");
	...
	alice.withdraw_dispute_stake(&mut runtime, U64(0), U64(0), Some(0)).expect("dispute stake claim failed");
}
...
fn test_cancel_dispute_participation_non_bonded_winning_outcome() {
	...
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(4))).expect("market resolution failed unexpectedly");
	alice.resolute_market(&mut runtime, U64(0), Some(1), U128(to_dai(5))).expect("market resolution failed unexpectedly");
	
	alice.dispute_market(&mut runtime, U64(0), Some(0), U128(to_dai(10))).expect("market dispute failed unexpectedly"); 
...
fn test_crowdsourced_dispute_correct_resolution() {
	...
	root.place_order(&mut runtime, U64(0), 1, U128(to_dai(5) / 50), U128(50), None).expect("order placement failed unexpectedly");
	root.place_order(&mut runtime, U64(0), 0, U128(to_dai(5) / 50), U128(50), None).expect("order placement failed unexpectedly");

	alice.set_allowance(&mut runtime, flux_protocol(), U128(to_dai(30))).expect("allowance couldn't be set");

	runtime.current_block().block_timestamp = market_end_timestamp_ns();
	
	carol.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(3))).expect("market resolution failed unexpectedly"); 
	alice.resolute_market(&mut runtime, U64(0), Some(0), U128(to_dai(2))).expect("market resolution failed unexpectedly"); 
	
	carol.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(5))).expect("market resolution failed unexpectedly"); 
	alice.dispute_market(&mut runtime, U64(0), Some(1), U128(to_dai(5))).expect("market resolution failed unexpectedly"); 
