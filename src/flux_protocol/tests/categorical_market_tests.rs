...
fn test_categorical_market_automated_matcher() {
	...
	accounts[0].set_allowance(&mut runtime, flux_protocol(), U128(to_dai(3_000_000))).expect("allowance couldn't be set");
	...
	accounts[0].set_allowance(&mut runtime, flux_protocol(), U128(to_dai(110_000_000))).expect("allowance couldn't be set");
