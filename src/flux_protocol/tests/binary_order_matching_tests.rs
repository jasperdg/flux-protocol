...
fn partial_binary_order_matching_test() {
	let (mut runtime, root, accounts) = init_runtime_env();
	accounts[0].set_allowance(&mut runtime, flux_protocol(), U128(to_dai(30))).expect("allowance couldn't be set");
	let tx_res = accounts[0].create_market(&mut runtime, empty_string(), empty_string(), 2, outcome_tags(0), categories(), U64(market_end_timestamp_ms()), 0, 0, "test".to_string()).unwrap();
	assert_eq!(tx_res.status, ExecutionStatus::SuccessValue(b"0".to_vec()));
	...
	let res = accounts[0].place_order(&mut runtime, U64(0), 1, U128(950000000000000000), U128(50), None).expect("order placement tx failed unexpectedly");
	let res = accounts[0].place_order(&mut runtime, U64(0), 1, U128(200000000000000000), U128(50), None).expect("order placement tx failed unexpectedly");
	let res = accounts[0].place_order(&mut runtime, U64(0), 1, U128(700000000000000000), U128(50), None).expect("order placement tx failed unexpectedly");
