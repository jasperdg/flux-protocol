use near_sdk::{near_bindgen, env, json_types};
use near_sdk::collections::UnorderedMap;
use json_types::{U128, U64};
...
// TODO: possible to change to markets to treemap since there is some sort of ordering
#[near_bindgen]
...
struct Markets {
	...
	nonce: u64,
	fdai_balances: UnorderedMap<String, u128>, // Denominated in 1e18
...
impl Markets {
	...
	pub fn add_to_creators_funds(
		...
		let mut balance = self.fdai_balances.get(&account_id).unwrap();
		balance += amount;
		self.fdai_balances.insert(&account_id, &balance);
	}
	...
	pub fn get_fdai_balance(&self, account_id: String) -> U128 {
		return self.fdai_balances.get(&account_id).unwrap_or(0).into();
	}
	...
	pub fn create_market(
		...
		let market_id = self.nonce.into();
		let hardcoded_resolution_fee = 1.into();
		...
		assert!(creator_fee_percentage <= self.max_fee_percentage.into());
		...
		let new_market = Market::new(market_id, account_id, description, extra_info, outcomes.into(), outcome_tags, categories, end_time.into(), creator_fee_percentage.into(), hardcoded_resolution_fee, affiliate_fee_percentage.into() ,api_source);
		self.markets.insert(&market_id, &new_market);
		self.nonce += 1;
		return market_id;
	}
	...
	pub fn place_order(
		...
		let account_id = env::predecessor_account_id();
		let balance: u128 = self.get_fdai_balance(account_id.to_string()).into();
		...
		let mut market = self.markets.get(&market_id).expect("market with id doesn't exist");
	...
	pub fn cancel_order(
		...
		let order = orderbook.open_orders.get(&order_id.into()).unwrap();
		assert!(account_id == order.creator);
		let to_return = orderbook.remove_order(order_id.into());
	...
	fn add_balance(
		...
		let balance = self.fdai_balances.get(&account_id);

		match balance {
			Some (balance) => self.fdai_balances.insert(&account_id, &(balance + amount)),
			None => self.fdai_balances.insert(&account_id, &(100 * one_dai)),
		};
	}
...
impl Default for Markets {
	fn default() -> Self {
		Self {
			...
			markets: UnorderedMap::new(vec![0]),
			nonce: 0,
			fdai_balances: UnorderedMap::new(vec![1]),
			max_fee_percentage: 5.into(),
			creation_bond: 0.into(),
		}
	}
}
...
mod tests {
	...
	fn to_dai(amt: u128) -> u128 {
		let base: u128 = 10;
		let amt: u128 = amt.into();
		return (amt * base.pow(17)).into();
	}
